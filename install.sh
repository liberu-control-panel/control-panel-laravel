...
setup_sudo_access() {
    ...
    # Determine the web server user based on OS
    local CP_USER
    case $OS in
        ubuntu|debian)
            CP_USER="www-data"
        ...
        almalinux|rhel|rocky|cloudlinux)
            ...
            ;;
        *)
            log_warning "Unknown OS '$OS'; defaulting Control Panel user to www-data"
            CP_USER="www-data"
            ;;
    esac
    ...
    # Write a targeted sudoers file granting the web server user NOPASSWD
    # access to the system commands used by the Control Panel services.
...
$CP_USER ALL=(root) NOPASSWD: /bin/chown -R www-data\:www-data /var/www/*
$CP_USER ALL=(root) NOPASSWD: /usr/bin/chown -R www-data\:www-data /var/www/*
$CP_USER ALL=(root) NOPASSWD: /bin/chown -R nginx\:nginx /var/www/*
$CP_USER ALL=(root) NOPASSWD: /usr/bin/chown -R nginx\:nginx /var/www/*
...
setup_standalone_app() {
    ...
    # Grant the web server user passwordless sudo access to the system
    # commands that the Control Panel uses to manage services
    setup_sudo_access
